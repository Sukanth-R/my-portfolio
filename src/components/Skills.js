    ...
    <section id="skills" className="py-24 px-4 sm:px-6 bg-black min-h-screen" style={{
      ...
      <style>
        {`
          ...
          .skills-glow-hover:hover {
            animation: skillsGlow 1.5s ease-in-out infinite;
          }
        `}
      </style>
        ...
        <div className="space-y-16">
          {skillCategories.map((category, catIndex) => (
            <div key={catIndex} className="space-y-8">
              <h3 className="text-transparent bg-clip-text bg-white text-2xl md:text-3xl font-bold text-center mb-8">
                {category.title}
              </h3>
              <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
                {category.skills.map((skill, skillIndex) => (
                  <div 
                    key={skillIndex}
                    className="relative flex flex-col items-center backdrop-blur-md p-4 sm:p-6 bg-gradient-to-br from-white/10 to-white/5 rounded-2xl border border-white/20 hover:border-red-800/50 skills-glow-hover transition-all duration-500 group transform hover:-translate-y-2 shadow-xl hover:shadow-2xl"
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-red-500/10 to-red-800/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl"></div>
                    ...
                    <h3 className="text-transparent bg-clip-text bg-white font-semibold text-sm sm:text-lg text-center z-10">
                      {skill.name}
                    </h3>
                  </div>
                ))}
